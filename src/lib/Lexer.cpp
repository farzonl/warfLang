...
std::unique_ptr<SyntaxToken> Lexer::NextToken() {
    ...
    if (type == SyntaxType::UnknownToken) {
        ...
        mPosition++;
        return std::make_unique<SyntaxToken>(type, tokenStartPos, std::string() + CurrentToken());
    }
    return std::make_unique<SyntaxToken>(type, tokenStartPos, SyntaxTokenToStrMap.at(type));
}